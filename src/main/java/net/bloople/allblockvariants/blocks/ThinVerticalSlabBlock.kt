...
open class ThinVerticalSlabBlock(settings: Settings) : Block(settings), Waterloggable {
    companion object {
        ...
        protected val WEST_SHAPE: VoxelShape = createCuboidShape(0.0, 0.0, 0.0, 4.0, 16.0, 16.0)
        protected val EAST_SHAPE: VoxelShape = createCuboidShape(12.0, 0.0, 0.0, 16.0, 16.0, 16.0)
        protected val NORTH_SHAPE: VoxelShape = createCuboidShape(0.0, 0.0, 0.0, 16.0, 16.0, 4.0)
        protected val SOUTH_SHAPE: VoxelShape = createCuboidShape(0.0, 0.0, 12.0, 16.0, 16.0, 16.0)
    ...
    private fun getVerticalSlabType(
        world: BlockView,
        pos: BlockPos,
        facing: Direction): VerticalSlabShape {
        ...
        //val thisOrientation = getAbsoluteOrientation(baseSlabType, facing).first()
        val neighbours = neighbourBlockStates.map { if(it != null) getAbsoluteOrientation(it) else null }
//            .map { if(it?.contains(thisOrientation) == true) it else null }
